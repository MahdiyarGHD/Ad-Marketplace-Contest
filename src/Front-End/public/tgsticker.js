window.RLottie = (() => {
		...
		if (apiInited) {
			...
			if (!apiInitStarted) {
				...
				for (
					...
					((workerNum) => {
						const rlottieWorker = new QueryableWorker(
							"/Ad-Marketplace-Contest/rlottie/rlottie-wasm.worker.js",
						);
						rlottieWorkers[workerNum] = rlottieWorker;
						rlottieWorker.addListener("ready", () => {
	...
	function initPlayer(el, options) {
		...
		const curDeviceRatio = options.maxDeviceRatio
		...
		if (options.returnData) return rlPlayer;
	...
	function render(rlPlayer, checkViewport) {
		...
		if (!rlPlayer.forceRender) {
			// not focused
			if (
				(!rlPlayer.playWithoutFocus && !document.hasFocus()) ||
				!rlPlayer.frameCount
			) {
				renderPlayer = false;
			}
