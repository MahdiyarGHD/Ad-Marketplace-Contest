    ...
    : Endpoint<Request, ErrorOr<Response>>
{
    ...
    public override async Task<ErrorOr<Response>> ExecuteAsync(Request req, CancellationToken ct)
    {
        ...
        var result = await campaignApplicationService.GetByCampaignIdAsync(req.CampaignId, req.Skip, req.Take);
        if (result.IsError)
            return result.Errors;

        var applications = result.Value.Select(a => new CampaignApplicationItem
        {
            Id = a.Id,
